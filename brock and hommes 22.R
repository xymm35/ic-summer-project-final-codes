...
MH = function(x0, sd1, sd2, sd3, sd4, Niter){
  count = 0
  #posterior
  f = function(x) likelihood_log(theta=x, Y=Y_mc)+dunif(x[1], min=0, max=2.5, log=T)+dunif(x[2], min=0, max=1.5, log=T)+dunif(x[3], min=0, max=2.5, log=T)+dunif(x[4], min=-1.5, max=0, log=T)
...
X = MH(c(0.91,1.41,2.01,-0.83),0.005,0.01,0.006,0.01,25000)
xall = X[,seq(1,25000-2000,by=50)]
write.table(xall, 'x22.csv', sep = ',')
...
con1 = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/con22.csv",head=T)
xall = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/x22.csv",head=T)
par(mfrow=c(2,4))
plot(density(as.numeric(xall[1,])), ylab = "density", xlab = "g_2", col = "#56B4E9",xlim=c(0,2.5), lwd = 2,  main=" ")
...
plot(density(as.numeric(xall[2,])), ylab = "density", xlab = "b_2", col = "#E69F00",xlim=c(0,1.5), lwd = 2,  main=" ")
...
plot(density(con1$g2), ylab = "density", xlab = "g_2", col = "#56B4E9", lwd = 2, xlim=c(-2,3.2), main=" ")
abline(v=0.6, col="red", lwd = 2, lty=2)
...
plot(density(con1$b2), ylab = "density", xlab = "b_2", col = "#E69F00", lwd = 2, xlim=c(0,3), main=" ")
abline(v=0.65, col="red", lwd = 2, lty=2)
...
plot(density(as.numeric(xall[4,])), ylab = "density", xlab = "b_3", col = "#CC79A7",xlim=c(-1.5,0), lwd = 2,  main=" ")
abline(v=-0.55, col="red", lwd = 2, lty=2)
...
plot(density(con1$g3), ylab = "density", xlab = "g_3", col = "#009E73", lwd = 2,xlim=c(-2,3),  main=" ")
abline(v=0.7, col="red", lwd = 2, lty=2)
...
plot(density(con1$b3), ylab = "density", xlab = "b_3", col = "#CC79A7", lwd = 2, xlim=c(-2,1), main=" ")
abline(v=-0.55, col="red", lwd = 2, lty=2)
...
mtext("parameter set 2 with Gaussian noise", line=-3, cex=2, outer = TRUE)
