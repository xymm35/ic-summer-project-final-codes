...
X = MH(c(0.91,1.41,2.01,-0.83),0.008,0.01,0.008,0.01,25000)
xall = X[,seq(1,25000-2000,by=50)]
write.table(xall, 'x2.csv', sep = ',')

pdf("trace1.pdf")

con1 = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/con2.csv",head=T)
xall = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/x2.csv",head=T)
par(mfrow=c(2,4))
...
abline(v=0.65, col="red", lwd = 3, lty=2)
...
plot(density(con1$g2), ylab = "density", xlab = "g_2", col = "#56B4E9", lwd = 2,xlim=c(0,2.5),  main=" ")
...
plot(density(con1$b2), ylab = "density", xlab = "b_2", col = "#E69F00", lwd = 2,xlim=c(0,1.5),  main=" ")
...
plot(density(con1$g3), ylab = "density", xlab = "g_3", col = "#009E73",xlim=c(0,2.5), lwd = 2,  main=" ")
abline(v=0.7, col="red", lwd = 3, lty=2)
abline(v=mean(con1$g3), col="#009E73", lwd = 2, lty=2)
lines(x=seq(0,2.5,length=10),rep(1/2.5,10), col="red", lwd = 2)
legend("topright", legend=c("posterior density", "posterior mean", "prior density", "true value"),
       col=c("#009E73", "#009E73", "red", "red"), lty=c(1,2,1,2))

plot(density(con1$b3), ylab = "density", xlab = "b_3", col = "#CC79A7",xlim=c(-1.5,0), lwd = 2,  main=" ")
abline(v=-0.55, col="red", lwd = 2, lty=2)
...
mtext("parameter set 2 with Cauchy noise", line=-3, cex=2, outer = TRUE)
