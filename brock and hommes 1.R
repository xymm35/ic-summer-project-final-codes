...
for(i in 1:2){
  ...
  for(j in 4:100){
    ...
    Y[i,j] = 1/R*(n1*(g1*Y[i,j-1]+b1)+n2*(g2*Y[i,j-1]+b2)+n3*(g3*Y[i,j-1]+b3)+n4*(g4*Y[i,j-1]+b4)+rnorm(1)*sig)
  }
}
...
X = MH(c(-0.93,-0.34,0.77,0.14),0.18,0.06,0.15,0.03,25000)
xall = X[,seq(1,25000-2000,by=50)]
write.table(xall, 'x1.csv', sep = ',')

pdf("trace1.pdf")

par(mfrow = c(2,2), oma = c(0, 1, 3, 0))
plot(xall[1,], type = "l", xlab = "iteration", ylab = "g_2", col = "#56B4E9", lwd = 2)
plot(xall[2,], type = "l", xlab = "iteration", ylab = "b_2", col = "#E69F00", lwd = 2)
plot(xall[3,], type = "l", xlab = "iteration", ylab = "g_3", col = "#009E73", lwd = 2)
plot(xall[4,], type = "l", xlab = "iteration", ylab = "b_3", col = "#CC79A7", lwd = 2)

mtext("traceplot", outer = T, font = 2, line = 0.6)

dev.off()


con1 = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/con1.csv",head=T)
xall = read.csv("C:/Users/dedaoyan/Desktop/xixi's project/x1.csv",head=T)

par(mfrow=c(2,4))
plot(density(as.numeric(xall[1,])), ylab = "density", xlab = "g_2", col = "#56B4E9",xlim=c(-2.5,0.2), lwd = 2,  main=" ")
...
plot(density(as.numeric(xall[2,])), ylab = "density", xlab = "b_2", col = "#E69F00",xlim=c(-1.5,0), lwd = 2,  main=" ")
...
plot(density(con1$g2), ylab = "density", xlab = "g_2", col = "#56B4E9", lwd = 2,  main=" ")
abline(v=-0.7, col="red", lwd = 2, lty=2)
...
plot(density(con1$b2), ylab = "density", xlab = "b_2", col = "#E69F00", lwd = 2,xlim=c(-1.5,0.2),  main=" ")
abline(v=-0.4, col="red", lwd = 2, lty=2)
...
plot(density(as.numeric(xall[3,])), ylab = "density", xlab = "g_3", col = "#009E73",xlim=c(-0.15,2.5), lwd = 2,  main=" ")
...
plot(density(con1$g3), ylab = "density", xlab = "g_3", col = "#009E73", lwd = 2,  main=" ")
abline(v=0.5, col="red", lwd = 2, lty=2)
...
plot(density(con1$b3), ylab = "density", xlab = "b_3", col = "#CC79A7",xlim=c(-0.2,1.5), lwd = 2,  main=" ")
abline(v=0.3, col="red", lwd = 2, lty=2)
...
mtext("parameter set 1 with Gaussian noise", line=-3, cex=2, outer = TRUE)
...
mean(as.numeric(xall[1,]))
mean(as.numeric(xall[2,]))
mean(as.numeric(xall[3,]))
mean(as.numeric(xall[4,]))

var(as.numeric(xall[1,]))
var(as.numeric(xall[2,]))
var(as.numeric(xall[3,]))
var(as.numeric(xall[4,]))